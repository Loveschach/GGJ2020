...
public static class Strings {
	public static Dictionary<string, string> GameStrings = new Dictionary<string, string>()
	{
		...
		{ "ART_fixBug_badSize", "LOL omg that thing with the bad size looked SO DUMB hahahahahaha. i'll change it (after i take a screenshot) xD" },
		{ "ART_fixBug_badTexture", "tbh i like the texture better that way, but i guess i gotta please the big guys lol. fixing!" },
		{ "ART_fixBug_badOrientation", "woah woah howd it get all turned around like that????? all good, ill fix it." },
		{ "ART_fixBug_badClipping", "i mean reaaaaaaaally... is clipping so bad?? jk good catch, that def needs to be fixed." },
